...
const rules = {
    ...
    shoulder: {
        ...
        sharp: { w: 90, h: 16 },      // Ã©paule droite
...
const labelMap = {
    ...
    shoulder: { sharp: 'Ã‰paules sharp', volume: 'Ã‰paules volume', fluid: 'Ã‰paules fluides' },

    col: { asym: 'Col asymÃ©trique', vneck: 'Col en V', oversize: 'Col oversize' },

    waist: { high: 'Taille haute', low: 'Taille basse', marked: 'Taille marquÃ©e' },
...
function describeVariant(params, index) {
    ...
    return `#${index} â€” ${labelFor('shoulder', params.shoulder)} | ${labelFor('col', params.col)} | ${labelFor('waist', params.waist)} | ${labelFor('length', params.length)} | ${labelFor('sleeve', params.sleeve)}`;
...
function generateSilhouettes() {
    ...
    if (valid < TARGET_SILHOUETTES) {
        ...
        if (valid < TARGET_SILHOUETTES) {
            ...
            alert(`Seulement ${valid}/${TARGET_SILHOUETTES} silhouettes valides gÃ©nÃ©rÃ©es (rÃ¨gles trop restrictives).`);
...
function createStatsChart(stats) {
    ...
    if (typeof Chart === 'undefined') {
        ...
        console.warn('Chart.js non chargÃ©, stats non rendues.');
    ...
    canvas.setAttribute('aria-label', `Diagramme des stats: ${valid} valides, ${rejected} rejetÃ©es, ${remaining} tentatives restantes`);
    ...
    window.statsChart = new Chart(canvas, {
        ...
        data: {
            ...
            labels: ['Silhouettes valides', 'Combinaisons rejetÃ©es', 'Tentatives restantes'],
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    ...
                    text: `Analyse: ${valid}/${TARGET_SILHOUETTES} valides â€” ${attempts}/${MAX_ATTEMPTS} essais`,
...
function updateInfo(baseParams, variations, stats) {
    ...
    const infoRows = [
        ...
        `<p><strong>Ã‰paules :</strong> ${labelFor('shoulder', baseParams.shoulder)}</p>`,
    ...
    const variationPreview = variations.length
        ...
        ? `Variations gÃ©nÃ©rÃ©es : ${variations

            .slice(0, 3)

            .map((v, idx) => describeVariant(v, idx + 1))

            .join(' Â· ')}${variations.length > 3 ? ' â€¦' : ''}`

        : 'Aucune silhouette valide nâ€™a Ã©tÃ© gÃ©nÃ©rÃ©e pour ces paramÃ¨tres.';
    ...
    configDetails.innerHTML = `
        ...
        <p><strong>Statut :</strong> ${valid}/${TARGET_SILHOUETTES} valides â€” ${rejected} rejetÃ©es sur ${attempts} essais.</p>
...
function getStyleDescription(params) {
    ...
    const desc = {
        ...
        shoulder: { sharp: 'Tension structurelle', volume: 'Volume architectural', fluid: 'DrapÃ© organique' },

        col: { asym: 'AsymÃ©trie audacieuse', vneck: 'Ligne Ã©purÃ©e', oversize: 'Enveloppement gÃ©nÃ©reux' }
...
console.log('Silhouette Engine prÃªt â€” rÃ¨gles, stats et export PDF actifs.');
